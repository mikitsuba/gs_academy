...
function searchCafe(array, coordinates, distance, offset) {
    return new Promise((resolve, reject) => {
        ...
        .done(function(response) {
            ...
            // // 1回のぐるなびAPIのAPIコールで取得できるのは100件まで。検索結果が100件を超えた場合にAPIコールを繰り返して全件を取得するために、再帰処理を入れた
            // offset++;
            // if (offset <= Math.ceil(response.total_hit_count / 100)) searchCafe(array, coordinates, distance, offset);
            resolve(array);
        })
        // Ajax通信が失敗した時
        .fail(function(error) {
...
function showResults(origin, cafes) {
    for (let i = 0; i < cafes.length; i++) {
        ...
        // レストラン情報の表示
        // google mapsのリンクは、これ（https://developers.google.com/maps/documentation/urls/get-started#directions-action）を使っている
        const html = `
        <div class="search_result" id="result_${i + 1}">
            ...
            <a href="https://www.google.com/maps/dir/?api=1&origin=${origin.lat},${origin.lng}&destination=${cafes[i].latlng.lat()},${cafes[i].latlng.lng()}&travelmode=${travelMode.toLowerCase()}" target="_blank" rel="noopener noreferrer" class="Googlemap_link">Goole Mapsで経路を調べる</a>
        </div>`
...
async function main() {
    ...
    // 取得したカフェにマーカーを設置する
    showResults(startCoordinates, searchHitCafes);
}
